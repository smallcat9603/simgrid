# v1, modified by smallcat 211207, add new objective (simulated time) for simgrid use
# v2, modified by smallcat 220623, add new objective (FLOPS) for HPL, HPCG (Kashiwa FPGA)
# v3, modified by smallcat 220706, handle output error & overtime 
# v4, modified by smallcat 220725, obtain output result when interrupted (hpl); note that read() will block until process is finished or killed, readline() will block until a new line is read, apply_async(func).get() will block until process is finished or killed
...
class MeasurementInterface(with_metaclass(abc.ABCMeta, object)):
    ...
    def call_program(self, cmd, limit=None, memory_limit=None, **kwargs):
        ...
        return {'time': float('inf') if killed else (t1 - t0),
                ...
                'stime': float('inf') if float(stime)==0.0 else float(stime),
                'hpltime': float('inf') if float(hpltime)==0.0 else float(hpltime),
