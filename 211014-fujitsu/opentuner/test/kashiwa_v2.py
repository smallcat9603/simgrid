...
class TestSimGridTuner(MeasurementInterface):
  ...
  def manipulator(self):
    ...
    # set env
    if self.args.appname == "hpl": # hpl-2.3:
  ...
  def run(self, desired_result, input, limit):
    ...
    if self.args.appname == "mm": # MM
      ...
      run_cmd += '-x OMP_NUM_THREADS=10 -x PATH -x LD_LIBRARY_PATH taskset -c 0-9 ' + mpi_bench_dir + 'hpl-2.3/bin/ompi/xhpl'
    elif self.args.appname == "hpcg": # intel mkl hpcg
      ...
      ppn = comb[3]
      num_threads = comb[4]
      run_cmd = 'OMP_NUM_THREADS={0} /home/proj/atnw/local/bin/mpirun -np 8 -hosts calc09,calc10,calc11,calc12,calc13,calc14,calc15,calc16 -ppn {1} '.format(num_threads, ppn) + mpi_bench_dir + 'hpcg/bin/xhpcg_mpich {0} {1} {2} 30'.format(nx, ny, nz)
    # else: # NPB
