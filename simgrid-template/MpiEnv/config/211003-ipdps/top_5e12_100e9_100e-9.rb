...
# modified by smallcat
# usage: 
# ruby top*.rb 2 2 2 //npb (modify bench_range if required), 256 nodes
# ruby top*.rb 2 2 1 //mm, himeno, graph500

require_relative "../common/benchtop"
require_relative "../common/generate_platform_fabien"
require "csv"
...
def main
 ...
 # bench_range=BENCHLOCKNAME.keys
  bench_range=["bt","cg","ft","lu","sp","mg"] #npb, too slow for is
  ...
  commonOpts[:power]=5e12        #The power of one core of simulated compute nodes in Flop/s 
  ...
  commonOpts[:switchbw]=5e12      #The bandwidth of switches in bit/s
  ...
  nswitches_range.each{ |nswitches|
    ...
    opts[:resultfile]="result_#{nswitches}_5e12_100e9_100e-9" 
	...
	#topology_range=["n256d4g", "n256d8g"]	
	topology_range=["n256d4g.edges.re", "n256d8g.edges.re"]
 ...
 #,{:headers=>true}
  ...
  tables.each{ |nswitches,table| 
    FileUtils.rm_rf  "2result_#{nswitches}_5e12_100e9_100e-9.csv"
    sleep(1)
    File.write("2result_#{nswitches}_5e12_100e9_100e-9.csv",table.to_csv({:col_sep=>"\t;"})) 
  }
end
