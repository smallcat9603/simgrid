...
/*
  8192/14.446249 = 567
  Number of iterations (nn*imax*jmax*kmax) = 14.446249
*/
#define BIGSIZE 567
#define VM1 3
#define VM2 63 
char bigdata[BIGSIZE];
...
/*  if(id == VM1){
    MPI_Irecv(bigdata, BIGSIZE, MPI_CHAR, VM2, 0, MPI_COMM_WORLD, &req);
    // プロセス1から受け取ったaの値を表示
    //printf("a = %s\n",bigdata);
  }
  if(id == VM2){
    MPI_Isend(bigdata, BIGSIZE, MPI_CHAR, VM1, 0, MPI_COMM_WORLD, &req2);
  }
*/
  //if(VM2 == id)
    //MPI_Waitall(1, &req2, &istatus);
  ...
  for(n=0 ; n<nn ; ++n){
    ...
    for(i=1 ; i<imax-1 ; ++i)
	  ...
	  /*ozaki*/
		  if(id == VM2){
                     MPI_Irecv(bigdata, BIGSIZE, MPI_CHAR, VM1, 100, MPI_COMM_WORLD, &req);
                     //受け取ったaの値を表示
                     //printf("a = %s\n",bigdata);
                   }
                   if(id == VM1){
                     MPI_Isend(bigdata, BIGSIZE, MPI_CHAR, VM2, 100, MPI_COMM_WORLD, &req2);
                   }
