...
float jacobi(int, MPI_Comm);
int initmax(int,int,int);
void initmt(int,int);
void initcomm(int,int,int,MPI_Comm*);
void sendp(int,int,int,MPI_Comm);
void sendp1();
void sendp2();
void sendp3();
...
main(int argc,char *argv[])
{
  ...
  //if(id==0){
  //sprintf(logfilename,"log_%i.txt",id);
  //logfile=fopen(logfilename,"w");
  //}

  initcomm(ndx,ndy,ndz,&mpi_comm_cart);
  it= initmax(mx,my,mz);
...
float
jacobi(int nn, MPI_Comm mpi_comm_cart)
{
  int i,j,k,n;
  ...
  for(n=0 ; n<nn ; ++n){
    ...
    sendp(ndx,ndy,ndz,mpi_comm_cart);
...
void
initcomm(int ndx,int ndy,int ndz, MPI_Comm* mpi_comm_cart)
{
  int  i,j,k,tmp;
  int  ipd[3],idm[3],ir;
  MPI_Comm  icomm;
  ...
  assert(MPI_Cart_create(icomm,
                  ndims,
                  idm,
                  ipd,
                  ir,
			 mpi_comm_cart)==MPI_SUCCESS);

  assert(mpi_comm_cart!=0);

  int ret=MPI_Cart_get(*mpi_comm_cart,
               ndims,
               idm,
               ipd,
		       iop);
  ...
  if(ndz > 1){
    assert(MPI_Cart_shift(*mpi_comm_cart,
                   2,
                   1,
                   &npz[0],
                   &npz[1])==MPI_SUCCESS);
 ...
 // fprintf(logfile,"Got topology x={%i,%i} y={%i,%i} z={%i,%i} \n",npx[0],npx[1],npy[0],npy[1],npz[0],npz[1]);
}
...
void
sendp(int ndx,int ndy,int ndz,MPI_Comm mpi_comm_cart)
{
  if(ndz > 1)
    sendp3(mpi_comm_cart);

  if(ndy > 1)
    sendp2(mpi_comm_cart);

  if(ndx > 1)
    sendp1(mpi_comm_cart);

}

void
sendp3(MPI_Comm mpi_comm_cart)
{
  MPI_Status   st[4];
  MPI_Request  req[4];
  ...
  //fprintf(logfile,"(3) Waiting for {%i,%i}\n",npz[0],npz[1]);
  MPI_Waitall(4,
              req,
              st);
}
...
sendp2(MPI_Comm mpi_comm_cart)
{
  ...
  //fprintf(logfile,"(2) Waiting for {%i,%i}\n",npy[0],npy[1]);
  MPI_Waitall(4,
              req,
              st);
}
...
sendp1(MPI_Comm mpi_comm_cart)
{
  ...
  //fprintf(logfile,"(1) Waiting for {%i,%i}\n",npx[0],npx[1]);
  MPI_Waitall(4,
              req,
              st);
}
