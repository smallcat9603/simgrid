...
long calcule_cout(long n, type_matrice & a, type_matrice & b, type_vecteur & p)
 {long i, j;
  long c = 0;
//   printf("calc ule_cout: ");
  for (i = 1; i <= n; i = i + 1) for (j = 1; j <= n; j = j + 1) {
    c = c + a[i][j] * b[p[i]][p[j]];
//     printf("a[%d][%d]*b[%d][%d]=%d*%d ", i, j, p[i], p[j], a[i][j], b[p[i]][p[j]]);
  }
//   printf("\n");
  return(c);
 }
...
void recuit(long n, type_matrice & a, type_matrice & b,
 ...
 {type_vecteur p;
  ...
  long cost = calcule_cout(n, a, b, p);
  meilleur_cout = cost;
...
//   cout << '\n';
  printf("@%d %d [", best_iter, meilleur_cout);
  for (i = 1; i <= n; i++) {
...
int main(int argc, char *argv[])
 {
  ...
  char trash[1000];
  if (argc < 3) {
    printf("QAP MinSum solver v2.01 (%s) by Ikki\n", __DATE__);
...
//   cin >> nb_iterations >> nb_res;
  ...
  scanf("%d", &n);
  if (n > n_max - 1) {
    printf("N=%d exceeds Nmax=%d. Exit.", n, n_max);
    return EXIT_FAILURE;
  };
  scanf("%[^\n]", trash);
