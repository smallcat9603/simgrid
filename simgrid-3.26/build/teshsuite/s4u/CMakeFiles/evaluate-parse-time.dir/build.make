# CMAKE generated file: DO NOT EDIT!
# Generated by "Unix Makefiles" Generator, CMake Version 3.16

# Delete rule output on recipe failure.
.DELETE_ON_ERROR:


#=============================================================================
# Special targets provided by cmake.

# Disable implicit rules so canonical targets will work.
.SUFFIXES:


# Remove some rules from gmake that .SUFFIXES does not remove.
SUFFIXES =

.SUFFIXES: .hpux_make_needs_suffix_list


# Suppress display of executed commands.
$(VERBOSE).SILENT:


# A target that is always out of date.
cmake_force:

.PHONY : cmake_force

#=============================================================================
# Set environment variables for the build.

# The shell in which to execute make rules.
SHELL = /bin/sh

# The CMake executable.
CMAKE_COMMAND = /usr/bin/cmake

# The command to remove a file.
RM = /usr/bin/cmake -E remove -f

# Escaping for special characters.
EQUALS = =

# The top-level source directory on which CMake was run.
CMAKE_SOURCE_DIR = /github/simgrid/simgrid-3.26

# The top-level build directory on which CMake was run.
CMAKE_BINARY_DIR = /github/simgrid/simgrid-3.26/build

# Include any dependencies generated for this target.
include teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/depend.make

# Include the progress variables for this target.
include teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/progress.make

# Include the compile flags for this target's objects.
include teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/flags.make

teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/evaluate-parse-time/evaluate-parse-time.cpp.o: teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/flags.make
teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/evaluate-parse-time/evaluate-parse-time.cpp.o: ../teshsuite/s4u/evaluate-parse-time/evaluate-parse-time.cpp
	@$(CMAKE_COMMAND) -E cmake_echo_color --switch=$(COLOR) --green --progress-dir=/github/simgrid/simgrid-3.26/build/CMakeFiles --progress-num=$(CMAKE_PROGRESS_1) "Building CXX object teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/evaluate-parse-time/evaluate-parse-time.cpp.o"
	cd /github/simgrid/simgrid-3.26/build/teshsuite/s4u && /usr/bin/c++  $(CXX_DEFINES) $(CXX_INCLUDES) $(CXX_FLAGS) -o CMakeFiles/evaluate-parse-time.dir/evaluate-parse-time/evaluate-parse-time.cpp.o -c /github/simgrid/simgrid-3.26/teshsuite/s4u/evaluate-parse-time/evaluate-parse-time.cpp

teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/evaluate-parse-time/evaluate-parse-time.cpp.i: cmake_force
	@$(CMAKE_COMMAND) -E cmake_echo_color --switch=$(COLOR) --green "Preprocessing CXX source to CMakeFiles/evaluate-parse-time.dir/evaluate-parse-time/evaluate-parse-time.cpp.i"
	cd /github/simgrid/simgrid-3.26/build/teshsuite/s4u && /usr/bin/c++ $(CXX_DEFINES) $(CXX_INCLUDES) $(CXX_FLAGS) -E /github/simgrid/simgrid-3.26/teshsuite/s4u/evaluate-parse-time/evaluate-parse-time.cpp > CMakeFiles/evaluate-parse-time.dir/evaluate-parse-time/evaluate-parse-time.cpp.i

teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/evaluate-parse-time/evaluate-parse-time.cpp.s: cmake_force
	@$(CMAKE_COMMAND) -E cmake_echo_color --switch=$(COLOR) --green "Compiling CXX source to assembly CMakeFiles/evaluate-parse-time.dir/evaluate-parse-time/evaluate-parse-time.cpp.s"
	cd /github/simgrid/simgrid-3.26/build/teshsuite/s4u && /usr/bin/c++ $(CXX_DEFINES) $(CXX_INCLUDES) $(CXX_FLAGS) -S /github/simgrid/simgrid-3.26/teshsuite/s4u/evaluate-parse-time/evaluate-parse-time.cpp -o CMakeFiles/evaluate-parse-time.dir/evaluate-parse-time/evaluate-parse-time.cpp.s

# Object files for target evaluate-parse-time
evaluate__parse__time_OBJECTS = \
"CMakeFiles/evaluate-parse-time.dir/evaluate-parse-time/evaluate-parse-time.cpp.o"

# External object files for target evaluate-parse-time
evaluate__parse__time_EXTERNAL_OBJECTS =

teshsuite/s4u/evaluate-parse-time/evaluate-parse-time: teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/evaluate-parse-time/evaluate-parse-time.cpp.o
teshsuite/s4u/evaluate-parse-time/evaluate-parse-time: teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/build.make
teshsuite/s4u/evaluate-parse-time/evaluate-parse-time: lib/libsimgrid.so.3.26
teshsuite/s4u/evaluate-parse-time/evaluate-parse-time: /usr/lib/aarch64-linux-gnu/libboost_context.so.1.71.0
teshsuite/s4u/evaluate-parse-time/evaluate-parse-time: teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/link.txt
	@$(CMAKE_COMMAND) -E cmake_echo_color --switch=$(COLOR) --green --bold --progress-dir=/github/simgrid/simgrid-3.26/build/CMakeFiles --progress-num=$(CMAKE_PROGRESS_2) "Linking CXX executable evaluate-parse-time/evaluate-parse-time"
	cd /github/simgrid/simgrid-3.26/build/teshsuite/s4u && $(CMAKE_COMMAND) -E cmake_link_script CMakeFiles/evaluate-parse-time.dir/link.txt --verbose=$(VERBOSE)

# Rule to build all files generated by this target.
teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/build: teshsuite/s4u/evaluate-parse-time/evaluate-parse-time

.PHONY : teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/build

teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/clean:
	cd /github/simgrid/simgrid-3.26/build/teshsuite/s4u && $(CMAKE_COMMAND) -P CMakeFiles/evaluate-parse-time.dir/cmake_clean.cmake
.PHONY : teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/clean

teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/depend:
	cd /github/simgrid/simgrid-3.26/build && $(CMAKE_COMMAND) -E cmake_depends "Unix Makefiles" /github/simgrid/simgrid-3.26 /github/simgrid/simgrid-3.26/teshsuite/s4u /github/simgrid/simgrid-3.26/build /github/simgrid/simgrid-3.26/build/teshsuite/s4u /github/simgrid/simgrid-3.26/build/teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/DependInfo.cmake --color=$(COLOR)
.PHONY : teshsuite/s4u/CMakeFiles/evaluate-parse-time.dir/depend

