#!/usr/bin/ruby -Ku
# 
# 	ラック配置→FSO端末配置→トポロジ生成→エッジの一部をFSO化
...
FSO_ADVANTAGE  = 3.33 / 5.0 # Light speed ratio of FSO to cable
FSO_HEADROOM   = 0.0 # Clearance between rack and ceiling [cm]
...
$help_message = <<-EOM
Usage:
  #{$0} [Options] Layout Input_file|Topology
...
Options:
  ...
  --headroom <H>
    	clearance between rack and ceiling [cm]
  --layout TYPE
    	...
    	default: mesh
  --fso SCHEME
    	longest => pick FSO links from the longest one
    	random  => pick FSO links randomly
    	default: random
  --seed <SEED>
    	random seed
...
Layout:
  <N>r<Z>f<T>
    	generate <N>-node layout with <Z>-sized racks; each rack has <T> FSO terminals

Input_file and/or Topologies:
  *.edges	per-node (non-clustered) edgelist file
...
def main
	...
	opt.on("--racksize Z") do |v|
		$opts[:racksize] = v
	end
	opt.on("--overhead OUTER,INNER") do |v|
		$opts[:overhead] = v
	end
	opt.on("--headroom H") do |v|
		$opts[:headroom] = v
	end
	opt.on("--layout TYPE") do |v|
		$opts[:layout] = v
	end
	opt.on("--fso SCHEME") do |v|
		$opts[:fso] = v
	end
	...
	if $opts[:headroom]
		$headroom = $opts[:headroom].to_i
	else
		$headroom = FSO_HEADROOM
	end
	...
	ARGV[1..-1].each do |topo|
		if File.exist?(topo)
			g.load_edgelist(topo)
		else
			g.wire(topo)
		end
	end
	# 一部のエッジをFSOに指定する
	fsomat = g.pick_fso_links
	# エッジリストとマップを書き出す
	g.save_edgelist(outfn, fsomat)
	g.save_layout(mapfn)
	# ラック単位のエッジリストを書き出す
	if $opts[:cluster]
...
class MyMap < MyGraph
	def initialize
		...
		@nfso_in = Hash.new # @nfso_in[ラック番号] = 使用済みFSO端末数
	end
	# レイアウトを生成
...
# 		STDERR.puts "<<<<<<<< make_layout (#{params}) >>>>>>>>"
	...
	def save_edgelist(filename, fsomat)
		open(filename, "w") do |f|
			each_edge_sorted do |n1, n2|
				if fsomat[n1, n2] == 1
					f.printf("%d %d %d\n", n1, n2, beam_between(n1, n2))
				else
...
# 			rckmat[r1, r2] += 1 # 同一ラック内も含める
	...
	# コストを表示
	def disp_cost(parameter, fsomat)
		...
		each_edge do |n1, n2|
			...
			if fsomat[n1, n2] == 1
				nlink_fso += 1
				cable_count += 2
				cable_amount += cable_of(0) * 2
				cable_cost += cost_of.cable(cable_of(0)) * 2
				fso_cost += cost_of.fso * 2
# 				puts "  #{n1}++#{n2}\t#{cable_of(0) * 2}cm\t$#{cost_of.cable(cable_of(0)) * 2}m\t$#{cost_of.fso * 2}"
			else
				cable_count += 1
...
# 			puts "  #{n}\tdeg=#{degree(n)}\t$#{cost_of.switch(degree(n))}"
		...
		total_cost = cable_cost + switch_cost + fso_cost
		STDOUT.puts format("%s\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t%d\t%d\t%0.2f\t%0.2f", parameter, total_cost, cable_cost, switch_cost, fso_cost, cable_amount, cable_count, cable_amount.to_f / cable_count, nlink_fso.to_f / nlink)
	end
	# 距離計算
	def calc_distances
		@racks.each do |r|
	...
	# FSOビーム長 (天井で反射する分だけ長く、無線区間のスピードアップ分だけ短くする)
	def beam_between(n1, n2)
		dist2 = dist_euclidean(@coord_of[@rack_of[n1]], @coord_of[@rack_of[n2]])
		dist3 = Math.sqrt(dist2 ** 2 + ($headroom * 2) ** 2)
		cable_of(dist3 * FSO_ADVANTAGE)
	end
	# 配線長
...
# 		STDERR.puts " => #{a1.length} links deleted. #{a1} = #{a1.uniq.length}. cable = #{@cable_total.to_f / 100}m."
	...
	# FSOに置き換えるリンクを選ぶ
	def pick_fso_links
		fsomat = MyMatrix.new # 0: cable, 1: FSO
		if $fso_choice == :longest
			pool = self.edges.sort_by{|n1, n2| dist_manhattan(@coord_of[@rack_of[n1]], @coord_of[@rack_of[n2]]) } # 配線長の昇順にソート
		else
			pool = self.edges.shuffle
		end
		until pool.empty?
			n1, n2 = pool.pop # 後ろから取り出す
			r1 = @rack_of[n1]
			r2 = @rack_of[n2]
			if r1 != r2 && @nfso_in[r1] < @racknfso && @nfso_in[r2] < @racknfso
				fsomat[n1, n2] = 1
				@nfso_in[r1] += 1
				@nfso_in[r2] += 1
			end
