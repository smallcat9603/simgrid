...
where
  ...
  <D>   	degree (ex. '6')
  <B>   	failure rate of the edgelist (ex. '0.25')
	...
	def wire(arg)
		...
		begin
			...
			when "hc"
				# hc    	hypercube, auto-sized
				# hc-<D>	hypercube
				nums[0] ||= (Math.log(@nodes.length) / Math.log(2)).ceil
				linecube([2] * nums[0])
			when "mesh"
				# mesh  	3-D mesh, auto-sized
				# mesh-<C>	<C>-D mesh
				nums = fullcube if nums.empty?
				linecube(nums)
			when "torus"
				# torus 	3-D torus, auto-sized
				# torus-<C>	<C>-D torus
...
# 				STDERR.puts "racks #{r1}--#{r2}\tnodes #{n1}--#{n2}\tdegs (#{degs[n1]})--(#{degs[n2]})"
	...
	def link_quasi_random_from(racks, deg, failrate, filename)
		raise "Too small degree (#{deg})" unless deg >= 0
		return if deg == 0
# 		STDERR.print "#{racks}"
		g = MyWeightedGraph.new
		g.load_edgelist(filename)
		pool = g.multiple_edges.random_drop(failrate)
		rdeg = deg * @racksize
		a0 = Array.new
		RAND_TRIAL.times do |t|
			a1 = Array.new
			degs = Hash.new(0)
			pool.shuffle.each do |r1, r2|
...
# 		STDERR.puts " => #{a0.length} links added. #{a0} = #{a0.uniq.length}"
	...
	def link_random_from(nodes, deg, failrate, filename)
		raise "Too small degree (#{deg})" unless deg >= 0
		return if deg == 0
		deg = nodes.length - 1 if deg >= nodes.length
# 		STDERR.print "#{nodes}"
		g = MyGraph.new
		g.load_edgelist(filename)
		pool = g.edges.random_drop(failrate)
		a0 = Array.new
		RAND_TRIAL.times do |t|
			a1 = Array.new
			degs = Hash.new(0)
			pool.shuffle.each do |n1, n2|
				if degs[n1] < deg && degs[n2] < deg
					a1.push([n1, n2])
					degs[n1] += 1
					degs[n2] += 1
...
# 		STDERR.puts " => #{a1.length} links added. #{a1} = #{a1.uniq.length}. total = #{@cable_total.to_f / 100}m."
	...
	# リンクをランダムに削除 (比率指定)
	def unlink_random(ratio)
		count = (self.edge_count * ratio).to_i
		self.edges.sample(count).each do |n1, n2|
			delete_link(n1, n2)
# 			STDERR.puts "Deleted #{n1}--#{n2}"
		end
	end
	# リンクをランダムに削除 (配線長指定)
