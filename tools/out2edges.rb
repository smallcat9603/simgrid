#!/usr/bin/ruby -Ku
# 
# 	レイアウト生成・トポロジ生成・クラスタリング
# 	Layout racks on a floor, generate a topology of nodes, and assigns clusters of the nodes to the racks
...
Options:
  -c                create per-rack edgelist clustered sequentially
  -m                create per-rack edgelist clustered optimally
  -u                create unidirectional (directed) arcs instead of edges
  -f                overwrite existing file
  -q                be quiet
  -v<LEVEL>         be verbose
  --floor=<ROWS>x<COLS>
                    number of racks on a floor (default: near to ROWS==COLS)
  --seed=<SEED>     random seed

Geometry:
  <N>z<Z>           put N nodes in N/Z racks each containing Z nodes
  <N>c<C>           put N nodes in C racks each containing N/C nodes
...
Files created:
  ...
  *.clu.edges       per-rack edgelist with number of cables (given -c)
  *.metis-*.edges   per-rack edgelist with number of cables (given -o)
...
def main
	$opts = ARGV.getopts("coufqv:", "floor:", "seed:", "help")
	$opts["v"] = $opts["v"].to_i
	seed = $opts["seed"].to_i
	...
	$myname = ARGV[0] + "." + tnames.join("+")
	$myname += ".seed=#{seed}" if $opts["seed"]
	srand(seed)
	
	ext_edges = ($opts["u"]) ? ".arcs" : ".edges"
	fnames = Hash.new
	fnames["coord"] = ARGV[0] + ".coord"
	fnames["edges"] = $myname + ext_edges
	fnames["clu.coord"] = $myname + ".clu.coord"
	fnames["clu.edges"] = $myname + ".clu.edges"
	fnames["mts.coord"] = $myname + ".metis-<Z>.coord"
	fnames["mts.edges"] = $myname + ".metis-<Z>.edges"
	if File.exist?(fnames["edges"]) && !$opts["f"]
		STDERR.puts "Output file '#{fnames['edges']}' exists. Use -f to overwrite" if !$opts["q"]
		exit(0)
	end
	g.make_geometry(ARGV[0], $opts["coord"])
	ARGV[1..-1].each do |arg|
		g.make_topology(arg)
	end
	g.save_geometry(fnames["coord"])
	g.save_edgelist(fnames["edges"])
	if $opts["c"]
		g.save_geometry(fnames["clu.coord"])
		g.save_edgelist_per_rack(fnames["clu.edges"])
	end
	if $opts["o"]
		g.optimize_clustering
		g.save_geometry(fnames["mts.coord"])
		g.save_edgelist_per_rack(fnames["mts.edges"])
	end
	g.disp_cost
end
...
# 			STDERR.puts "  #{n}\tdeg=#{degree(n)}\t$#{cost_of_switch(degree(n))}" if $opts["v"] >= 4
		...
		STDOUT.puts format("%s\t%0.2f\t%0.2f\t%0.2f\t%d\t%d\t%d\t%0.2f\t%0.2f", $myname, total_cost, cable_cost, switch_cost, @racksize, cable_amount, cable_count, cable_amount.to_f / cable_count, switch_power)
	end
...
class MyMap < MyGraph
	...
	def make_geometry(arg, floor)
		case arg
		when /(\d+)[zr](\d+)/ # "r" is for backward compatibility
			nnodes = $1.to_i # ノード数
		...
		when /(\d+)c(\d+)/
			nnodes = $1.to_i # ノード数
			nracks = $2.to_i # ラック数
			@racksize = (nnodes.to_f / nracks).ceil # ラックサイズ
		else
			raise "Undefined geometry notation '#{arg}'"
		end
		case floor
		when /(\d+)\D+(\d+)/
			nx = $1.to_i # 奥行方向のラック数
...
# 			commvol = res.scan(/communication volume: *(\d+)/)[0][0].to_i
		...
		if @racksize < maxsize
			STDERR.puts "Warning: #{$myname}: Partition size (#{maxsize}) exceeds rack size (#{@racksize})" if !$opts["q"]
	...
	def save_geometry(filename)
		open(filename.gsub("<Z>"){ @racksize }, "w") do |f|
	...
	def save_edgelist_per_rack(filename)
		...
		grack.save_edgelist(filename.gsub("<Z>"){ @racksize })
	end
	# 配線長
...
class MyNetwork < MyMap
	...
	def make_topology(arg)
		...
		when "sky" # Skywalk: Z方向ノード間ランダム結合＋Y/X方向ラック間ランダム結合
			...
			if params[1].to_i > ds_max
				link_quasi_random(@racks, @racksize, params[1].to_i - ds_max) do |r1, r2|
					@coord_of[r1][0] != @coord_of[r2][0] && @coord_of[r1][1] != @coord_of[r2][1]
				end
			end
	...
	def link_random(nodes, deg, unidir = false)
		...
		STDERR.puts "Warning: #{$myname}: Too large degree (#{deg})" unless deg < nodes.length if !$opts["q"]
		best = nil
